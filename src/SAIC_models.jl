...
@export function MassControl(;
                            ...
                            k_5 = 2.0,             # Rates for X3
                            k_div = 0.005,         # Rate  for cell division
                            k_0 = 0.0025)          # Rate  for cell death
    S = System("MassControl", 1+2+3, # environment or cell , Z1 & Z2, X1 & X2 & X3
            Dict(1 =>[0,0,0,0,0,0], #
                 2 =>[0,1,0,0,0,0], # Total Z1 mass (in medium)
                 3 =>[0,0,1,0,0,0], # Total Z2 mass (in medium)
                 4 =>[1,0,0,0,0,0], # Number of cells
                 5 =>[1,0,0,1,0,0], # Total X1 mass (in cell)
                 6 =>[1,0,0,0,1,0], # Total X2 mass (in cell)
                 7 =>[1,0,0,0,0,1], # Total X3 mass (in cell)
                )
            )

    # SAIC reactions
    reference = new_chemical_reaction_class([0,1,0,0,0,0], omega)
    reference.H = (n::Matrix{Int64}, Mom::Vector{Int64}) -> 1
    reference.fast_sample_reactants! = fast_sample_first

    measurement = new_chemical_reaction_class([0,0,1,0,0,0], theta)
    ...
    comparison = new_chemical_reaction_class([0,-1,-1,0,0,0], eta)
    ...
    actuation = new_chemical_reaction_class([0,0,0,1,0,0], k_a)
    actuation.H = (n::Matrix{Int64}, Mom::Vector{Int64}) -> Mom[2]*Mom[4] # Catalyzed by Z1 (without 1/N)
    actuation.fast_sample_reactants! = fast_sample_uniform_cell

    # cells' reaction network
    prodX1 = new_chemical_reaction_class([0,0,0,1,0,0], k_1)
    prodX1.H = (n::Matrix{Int64}, Mom::Vector{Int64}) -> Mom[4]
    prodX1.fast_sample_reactants! = fast_sample_uniform_cell

    deathX1 = new_chemical_reaction_class([0,0,0,-1,0,0], k_2)
    deathX1.H = (n::Matrix{Int64}, Mom::Vector{Int64}) -> Mom[5]
    deathX1.fast_sample_reactants! = fast_sample_mass_x1

    prodX2 = new_chemical_reaction_class([0,0,0,0,1,0], k_3)
    prodX2.H = (n::Matrix{Int64}, Mom::Vector{Int64}) -> Mom[5] # Catalyzed by X1
    prodX2.fast_sample_reactants! = fast_sample_mass_x1

    deathX2 = new_chemical_reaction_class([0,0,0,0,-1,0], k_4)
    deathX2.H = (n::Matrix{Int64}, Mom::Vector{Int64}) -> Mom[6]
    deathX2.fast_sample_reactants! = fast_sample_mass_x2

    prodX3 = new_chemical_reaction_class([0,0,0,0,0,1], k_5)
    prodX3.H = (n::Matrix{Int64}, Mom::Vector{Int64}) -> Mom[4] # Catalyzed by N
    prodX3.fast_sample_reactants! = fast_sample_uniform_cell

    # Compartment reactions
    cell_division = TransitionClass(1, 2, k_div) # Cell division
    cell_division.H = (n::Matrix{Int64}, Mom::Vector{Int64}) -> Mom[7] # Catalyzed by X3
    ...
    cell_death.H = (n::Matrix{Int64}, Mom::Vector{Int64}) -> div(Mom[4]*(Mom[4]-1), 2) # This is binary death
    cell_death.fast_sample_reactants! = fast_sample_uniform_cell

    add_transition_class(S, reference, measurement, comparison, actuation,
                            prodX1, deathX1, prodX2, deathX2, prodX3,
                            cell_division, cell_death)
...
@export function NumberControl(;
                                ...
                                k_div = 0.005,         # Rate  for cell division
                                k_0 = 0.0025)          # Rate  for cell death
    S = System("NumberControl", 1+2+2, # environment or cell , Z1 & Z2, X1 & X2
            Dict(1 =>[0,0,0,0,0], #
    ...
    cell_division.fast_sample_reactants! = fast_sample_cell_divNC

    cell_death = TransitionClass(1, 0, k_0) # This is a cell death
    cell_death.H = (n::Matrix{Int64}, Mom::Vector{Int64}) -> div(Mom[4]*(Mom[4]-1), 2) # This is binary death
    cell_death.fast_sample_reactants! = fast_sample_uniform_cell

    add_transition_class(S, 
                            reference, measurement, comparison, actuation, 
                            prodX1, deathX1, prodX2, deathX2, 
                            cell_division, cell_death)
...
function fast_sample_cell_divNC(r_indices::Vector{Int64}, n::Matrix{Int64}, Mom::Vector{Int64})
    rv = rand()*Mom[6]
    r_indices[1] = 2 # the first one is environment!
    val = 1.0*n[5,2]
    while val < rv
